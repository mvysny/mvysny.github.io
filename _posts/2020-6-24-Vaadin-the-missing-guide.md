---
layout: post
title: Vaadin 14 - The missing guide
---
...
Just keep in mind to have the `flow-build-info.json` file on the classpath EXACTLY ONCE,
and always coming from your main project. It often happens that some plugin is
packaged incorrectly and include its
own `flow-build-info.json` from their example projects; in such case Vaadin can not
decide properly which file it will pick, and will use clever magic to try to pick
the main project one. Sometimes the magic backfires and an incorrect file is chosen.

Such plugins need to be fixed and the `flow-build-info.json` file removed from their jar files.
