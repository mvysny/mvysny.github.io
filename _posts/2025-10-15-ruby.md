...
on the machine. The two extra packages save you a world of pain later:
...
* `ruby-bundler` gives you the `bundle` command, which pretty much every Ruby project uses to manage
  its dependencies.
...
Default gems get updated along with the `ruby` package itself, so I don't think this is necessary.
